...
function readConfig(config: Record<string, unknown>): {
  ...
  return { publicKey, amount, currency, buyerEmail, buyerPhoneNumber };
}
...
export const paidySdkLauncher: SdkLauncher = {
  ...
  async launch(config: Record<string, unknown>): Promise<SdkLaunchOutcome> {
    ...
    const { publicKey, amount, currency, buyerEmail, buyerPhoneNumber } =
      readConfig(config);
    ...
    return new Promise<SdkLaunchOutcome>((resolve) => {
      ...
      const payload: PaidyLaunchPayload = {
        ...
        buyer: {
          // name1 は必須。サーバー側で渡せないケースがあるためスペース 1 文字でフォールバック。
          // 実運用では purchase ページの user 情報を sdkLauncher 用 config に流す経路を追加する想定。
          name1: " ",
          email: buyerEmail,
