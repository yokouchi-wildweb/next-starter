...
- マッピング: `addressLine1→line1` / `addressLine2→line2` / `locality→city` / `administrativeArea→state` /
  `postalCode→zip` / `country→country`（既定 `"JP"`）。
- **Paidy 仕様で `shipping_address` を送る場合 line1 と zip が必須**。`addressLine1` と `postalCode`
  が両方揃う時のみ付与する（不完全だと Paidy が 400 を返し決済全体が失敗するため）。
