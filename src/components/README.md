...
- **主な役割**：どのカテゴリにも属さない小規模な汎用部品を格納します。単体で完結する装飾タグや表示専用のラベルなど横断的に使い回せる要素をまとめています。
- **ガイドライン**
  - 明確なカテゴリが決まるまでの一時置き場にせず、汎用的に利用される粒度の小さい部品のみを追加します。

## DataTable

- **主な役割**：表形式のデータ表示コンポーネント群。テーブル本体の土台に加えて、ヘッダー・セル・ツールバーなど表レイアウトを構成するラッパーを提供します。
- **ガイドライン**
  - テーブル表示や操作に関係する要素のみを配置します。
...
- **主な役割**：ユーザーへ処理状況を伝えるスピナーやスケルトンなどのコンポーネントを管理します。
- **ガイドライン**
  - ローディング状態や結果通知など、フィードバック表示に特化した要素のみを追加します。
...
- **主な役割**：モーダルやダイアログなど画面上に重ねて表示するコンポーネントを扱います。
- **ガイドライン**
