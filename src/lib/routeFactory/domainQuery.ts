// src/lib/routeFactory/domainQuery.ts
//
// 汎用ドメインルート（/api/[domain]/**）のクエリパラメータ解析ヘルパー。
// app 層からここへ集約し、汎用ルート・固定ドメインファクトリの双方が共有する。
// 旧 `@/app/api/[domain]/search/utils` は本モジュールの再エクスポートに置き換わっている。
