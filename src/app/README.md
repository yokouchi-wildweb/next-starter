...
| `(user)/` | ユーザー向けフロントエンド |
| `admin/` | 管理者向け画面 |
| `debug/` | デバッグ用画面（管理者カテゴリ + デバッガーロール限定、layout が一括ガード） |
| `api/` | API ルート |
...
| `auth/` | 認証関連 |
| `admin/` | 管理者専用API |
| `debug/` | デバッグ用API（`ADMIN_OR_DEBUGGER_RULE` で認可、`ping/` が雛形） |
| `storage/` | ファイル操作 |
| `wallet/` | ウォレット操作 |
| `webhook/` | 外部サービス連携 |
...
4. デバッグ用（管理者 + デバッガーのみに見せたい）?
   - はい → debug/（layout が admin カテゴリ + debugger ロールで一括ガード）
```
