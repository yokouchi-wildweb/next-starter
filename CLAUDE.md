...
deploy: pnpm deploy:all (unified app+worker, manual-ops standard, room disabled→app only) | pnpm room:init (setup) | pnpm room:deploy (worker only, disabled→skip) | pnpm room:check (post-deploy verification, 11 checks) | pnpm room:dev (local, no CF account) | secret put AFTER first deploy (pre-deploy put = interactive prompt trap) | dev must NOT share prod worker (room state is shared per ns/roomId) | ref: servers/room/README.md
