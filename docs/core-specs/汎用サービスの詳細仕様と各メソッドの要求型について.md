...
| メソッド | 受け取る型 | 主な処理 | 備考 |
| --- | --- | --- | --- |
| `create(data)` | `Insert`（= `TCreate`） | `uuid` 付与や `id` 削除など ID 採番の正規化を行い、`undefined` を `null` に変換して挿入します。 | ID の扱いは `serviceOptions.idType` で制御します。 |
| `list()` | なし | 既定のソート条件を適用して全件取得します。 | `defaultOrderBy` があれば適用。 |
| `get(id)` | `string` | 単一 ID のレコードを取得します。 | 見つからなければ `undefined`。 |
| `update(id, data)` | `Partial<Insert>` | 渡された部分データから `undefined` を除き、`null` 正規化後に更新します。 | 部分更新専用。 |
...
| `bulkDelete(ids)` | `string[]` | 複数 ID を `inArray` で削除します。 | |
| `upsert(data, options?)` | `Insert & { id?: string }` | `create` と同様に挿入データを正規化し、`onConflictDoUpdate` の `set` に同じ型のデータを渡します。`id` は `set` から除去されます。 | 衝突対象は `resolveConflictTarget` で決定。 |
...
- 既存レコードの部分更新だけを行いたい場合は `update(id, Partial<Insert>)` を利用し、部分的な入力を `omitUndefined` → `normalizeUndefinedToNull` で正規化して更新します。

### 3.2 `upsert` の挙動詳細

1. `Insert` 型データをコピーし、ID 採番ポリシーに従って `id` を調整します。
2. `omitUndefined` で `undefined` を取り除き、`normalizeUndefinedToNull` で `null` に揃えます。
