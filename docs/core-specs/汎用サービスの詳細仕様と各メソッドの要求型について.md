...
| `search(params)` | `SearchParams & ExtraWhereOption` | 条件組み立て・あいまい検索・ページングを行います。`extraWhere` で Drizzle SQL 条件を直接注入可能。 | `defaultSearchFields` を活用。 |
...
| `reorder(id, afterItemId)` | `string, string \| null` | 指定レコードの `sort_order` を `afterItemId` の直後に移動します。Fractional Indexing を使用。 | `sortOrderColumn` 設定が必要。 |
| `initializeSortOrder(ids)` | `string[]` | 指定 ID 配列の `sort_order` を初期化します。既存の最大値の後に順次配置。 | `sortOrderColumn` 設定が必要。 |
| `searchForSorting(params)` | `SearchParams & ExtraWhereOption` | `search` と同様だが、結果に `sort_order = NULL` のレコードがあれば自動初期化してから返却。`extraWhere` も伝播する。 | ソート画面での使用を推奨。 |
