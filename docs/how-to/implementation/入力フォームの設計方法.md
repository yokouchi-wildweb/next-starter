...
- Shadcn UI の `Form` コンポーネント（`FormProvider`）と `<form>` を統合した共通コンポーネント `<AppForm>` を利用します。
...
1. **フォームコンテナ** – `useForm` で生成したインスタンスと送信ハンドラを `<AppForm methods={form} onSubmit={handleSubmit}>` に渡す。
...
3. **`AppForm` コンポーネントで `<form>` を描画する**
   ```tsx
   const handleSubmit = async (values: FormValues) => {
     // 送信処理をここに記述
   };

   <AppForm methods={form} onSubmit={handleSubmit} className="space-y-4" noValidate>
     {/* フィールドをここに並べる */}
   </AppForm>
   ```
   - `pending` に通信中フラグを渡すと、`formState.isSubmitting` と合わせてフォーム全体を自動的に無効化できます。
   - `preventSubmitOnEnter` は既定で有効になっており、検索など Enter 送信を許可したい入力は `data-allow-enter` を付与して除外します。
   - `disableWhilePending` を `false` にすると、送信中でもフィールドの入力を継続できます（デフォルトは自動で `<fieldset disabled>` を適用）。
...
- `handleSubmit`: `<AppForm>` コンポーネント内部で利用され、バリデーションと送信処理をまとめてハンドリングします。独自の送信制御が必要な場合のみ直接呼び出してください。
