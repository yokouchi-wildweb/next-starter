...
| `npm run dc:add -- <Domain>` | 既存 `domain.json` にフィールドを追記する。 |
...
- `components/presenters.ts` … 管理テーブルや詳細表示向けのフォーマッタ定義。`generateFiles` で `presenters` を選択すると生成される
...
### 既存ドメインへフィールドを追加したい場合
1. `domain.json` を更新する。`npm run dc:add -- <Domain>` を使うと既存設定を保持したまま末尾へフィールドを挿入できる。
2. `npm run dc:generate -- <Domain>` 実行時に「生成カテゴリを手動で選択」を選び、`entities`／`constants`／`types` のみにチェックを入れて再出力する。フォーム側は DomainFieldRenderer が `domain.json` を参照して自動描画するため、CRUD フックやサービス層だけでなくコンポーネントの再生成も不要。
3. 生成後は `git status` で差分を確認し、エンティティ・定数・型が新フィールドを反映していることを確認する。
