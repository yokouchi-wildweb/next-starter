...
- Node.js 20.9 以上 (Next.js 16 系の動作要件)
- npm 10 以上
...
- リモートメディアをブラウザで読み取る機能（動画の MSE 連結再生・canvas でのサムネ生成・Web Audio 等）を使う場合は、Storage バケットに CORS を設定する（バケット単位・一度きり。手順: [Storage の CORS 設定（リモートメディア読み取りの基盤前提）](StorageのCORS設定（リモートメディア読み取りの基盤前提）.md)）
