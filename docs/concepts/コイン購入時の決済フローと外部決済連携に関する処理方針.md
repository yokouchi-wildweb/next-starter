...
アプリ内通貨（コイン、ポイント等）の購入機能における決済フロー、状態管理、二重購入防止の設計をまとめます。

> **関連ドキュメント**: ウォレットの基本設計については [ウォレット機能の設計方針と運用ガイド](./ウォレット機能の設計方針と運用ガイド.md) を参照。
...
| `id` | UUID | リクエストID（主キー） |
| `user_id` | UUID | 購入ユーザー |
| `idempotency_key` | UUID | 冪等キー（UNIQUE制約、二重購入防止） |
| `wallet_type` | ENUM | 対象ウォレット種別（`regular_coin` 等） |
| `amount` | INTEGER | 購入数量（コイン数など） |
| `payment_amount` | INTEGER | 支払い金額（円） |
| `payment_method` | TEXT | 支払い方法（`credit_card` / `amazon_pay` / `convenience_store` / `bank_transfer`） |
| `status` | ENUM | `pending` → `processing` → `completed` / `failed` / `expired` |
| `payment_provider` | TEXT | 決済プロバイダ（`dummy` / `stripe` / `paypay` 等） |
| `payment_session_id` | TEXT | 外部決済サービスのセッションID（nullable） |
| `wallet_history_id` | UUID | 完了時に作成された wallet_history のID（nullable） |
| `error_code` | TEXT | エラー時のコード（nullable） |
| `error_message` | TEXT | エラー時の詳細メッセージ（nullable） |
| `milestone_results` | JSONB | マイルストーン評価結果（nullable、`PersistedMilestoneResult[]`） |
...
│    │  ├─ adjustBalance() でウォレット残高を更新 ← 【権威あるソース】       │
│    │  └─ purchase_request 更新（status: completed, wallet_history_id）   │
│    │                                                                      │
│    [ユーザー経路] ────────────────────────────────────────────────────    │
│    │  リダイレクト: /coins/purchase/callback?request_id=xxx              │
│    │  └─ purchase_request の status を確認                               │
│    │     ├─ completed → /coins/purchase/complete へリダイレクト          │
│    │     ├─ processing → ポーリングで Webhook 完了を待機                  │
│    │     └─ failed/expired → /coins/purchase/failed へリダイレクト       │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| ページ | パス | 役割 |
| --- | --- | --- |
| 購入内容確認 | `/coins/purchase` | 購入数量・支払い方法を選択 |
| コールバック | `/coins/purchase/callback` | 決済後の状態検証・振り分け |
| 完了 | `/coins/purchase/complete` | 購入成功の表示 |
| 失敗 | `/coins/purchase/failed` | 購入失敗・エラーの表示 |

---

## 🔄 3. Callback と Webhook の役割分担

Callback ページと Webhook は**補完関係**にあり、両方が必要。

| 観点 | Callback ページ | Webhook |
| --- | --- | --- |
| 主な役割 | ユーザーへの即時フィードバック | 確実な決済完了の記録 |
| 到達保証 | ❌ ユーザーが戻らない可能性あり | ✅ サーバー間通信＋リトライ機構 |
| 非同期決済 | ❌ 対応不可（コンビニ払い等） | ✅ 完了時に通知される |
| セキュリティ | △ URLパラメータは改ざん可能 | ✅ 署名検証で保証 |
| 残高更新 | ❌ 行わない（参照のみ） | ✅ ここで実行 |

**重要**: 実際のコイン付与（残高更新）は必ず Webhook 側で行う。Callback ページは DB の状態を確認して表示するだけ。

### Callback ページが必要な理由

1. **Webhook との競合状態の吸収**
   - ユーザーがリダイレクトで戻るタイミングと Webhook 到着タイミングは不定
   - Callback ページでポーリングして Webhook 完了を待機できる

2. **状態検証の一元化**
   - セッションと purchase_request の照合
   - 不正アクセス防止（直接 complete ページにアクセスさせない）
...
function CallbackPolling({ requestId }: { requestId: string }) {
  ...
  return <LoadingSpinner message="決済結果を確認中..." />;
}
```
...
- [ ] `evaluateMilestones()` でマイルストーン評価を実行しているか
- [ ] マイルストーン結果を `purchase_requests.milestone_results` に保存しているか

### コールバックページ
- [ ] `request_id` パラメータを取得しているか
- [ ] ログインユーザーと `purchase_request.user_id` を照合しているか
- [ ] `status` に応じて適切なページにリダイレクトしているか
- [ ] `processing` 状態の場合、ポーリングで Webhook 完了を待機しているか
- [ ] ポーリングのタイムアウト処理を実装しているか
- [ ] タイムアウト時にサポート案内を表示しているか

### 完了後
- [ ] 完了ページで購入内容（数量・金額）を表示しているか
- [ ] マイルストーン達成通知が表示されているか（`milestoneResults` が存在する場合）
- [ ] トーストで購入完了を通知しているか
- [ ] 残高表示を更新しているか（SWR再検証）
- [ ] コイン管理ページへの導線を用意しているか
...
| ページ | ファイル | 役割 |
| --- | --- | --- |
| コイン管理 | `src/app/(user)/(protected)/coins/page.tsx` | 残高表示、購入オプション一覧 |
| 購入確認 | `src/app/(user)/(protected)/coins/purchase/page.tsx` | 購入数量・支払い方法選択 |
| コールバック | `src/app/(user)/(protected)/coins/purchase/callback/page.tsx` | 決済後の状態検証、ポーリング待機 |
| 完了 | `src/app/(user)/(protected)/coins/purchase/complete/page.tsx` | 購入成功メッセージ |
| 失敗 | `src/app/(user)/(protected)/coins/purchase/failed/page.tsx` | エラー表示、サポート案内 |
